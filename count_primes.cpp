...
// Composite function to calculate total number of left-truncatable primes
int count_left_trunc_primes(const std::vector<unsigned long long> &all_primes_array,
                             ...
                             const std::vector<bool> &prime_bitset, int digits)
{
    ...
    for (size_t i = 0; i < all_primes_array.size(); ++i)
    {
        ...
        long long int temp_prime = current_prime;
        ...
        {
            int leftmost = temp_prime / current_digits;
            // Check if the current truncated number is prime using bitset
            if (temp_prime >= prime_bitset.size() || !prime_bitset[temp_prime] || leftmost == 0)
            {
                is_l_truncatable = 0; // Not a left-truncatable prime
                break;
            }

            temp_prime %= current_digits;
            current_digits /= 10;
        }
...
int main(int argc, char *argv[])
{
    ...
    // Calculate total number of left-truncatable primes up to the specified number of digits
    int total_count = 0;
    for (int i = digits; i > 0; i--)
    {
        int count = count_left_trunc_primes(all_primes_array, primes_per_digit, prime_bitset, i);
        if (count < 0)
        {
            fprintf(stderr, "Error counting left-truncatable primes for %d digits.\n", i);
            return 1;
        }
        total_count += count;
        printf("Number of %d-digit left-truncatable primes: %d (n = %llu)\n", i, count, primes_per_digit[i]);
    }
    printf("\nTotal number of left-truncatable primes up to %d digits: %d (n = %zu)\n\n", digits, total_count, all_primes_count);
